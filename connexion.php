	...
	{	
		...
		else //On check le mot de passe
		{
			...
			if(count($iden) == 1)
			{
				...
				if ($iden[0]['mdp'] == sha1($_POST['mdp']) && preg_match("#^[a-z]+.[-a-z]{2,}+@votreemail.fr$#",$_POST['email'])) // OK !
				{
					$_SESSION['email'] = $iden[0]['mail'];
					$_SESSION['id'] = $iden[0]['id_personne'];
					$_SESSION['droit'] = $iden[0]['droit'];
					setcookie('email', $iden[0]['mail'], time() + 365*24*3600, null, null, false, true);
					setcookie('drt', $iden[0]['droit'], time() + 365*24*3600, null, null, false, true);
