<?php 
	...
	{
		//print_r($_POST);
		$line = count($_POST['datec']);
		$_POST['nblignes']=$line;
		for($i=0;$i<$line;$i++)
		{
			if($_POST['exp'][$i]=='' && $_POST['contenu'][$i]=='' && $_POST['tech'][$i]=='')
			{
				unset($_POST['datec'][$i]);
				unset($_POST['exp'][$i]);
				unset($_POST['contenu'][$i]);
				unset($_POST['tech'][$i]);
				$_POST['nblignes']=$_POST['nblignes']-1;
			}
			
		}
		//print_r($_POST);
		
		$_POST['datec']=array_values($_POST['datec']);
		$_POST['exp']=array_values($_POST['exp']);
		$_POST['contenu']=array_values($_POST['contenu']);
		$_POST['tech']=array_values($_POST['tech']);
		// print_r($_POST);
		// die();
		$erreur=array();
		...
		if(count($erreur)!=0)
		{
			$erreurmsg='Il y a des erreurs. Merci de vérifier les données rentrées.';
		}
		else
		{	for($j=0; $j<$_POST['nblignes'];$j++)
			{
				...
				header('Location: ajouterarrive.php?msg=ok');
			}
		}
	}
?>
...
var twoDigitDay = ("0" + d.getDate()).slice(-2);
	...
	/* Bloque la validation des formulaires via la touche entrée */
...
<section id="content">
	...
	<?php if(isset($_GET['msg']) && $_GET['msg']=='ok'){ echo '<p class="msgok left">La mise à jour s\'est bien déroulée.</p><br/>'; }?>
		...
		<fieldset>
			<ul id="conteneur">
				...
				<li id="element1" class="clear">
					<ul>
						<li class="classdate" ><input type="text" value="<?php if(isset($_POST['datec'][0]) && $_POST['datec'][0]!='') {echo $_POST['datec'][0];} else echo date('d/m/Y'); ?>" name="datec[]" id="datec1" class="datec"/><?php if(isset($erreur['datec'][0])) echo '<p class="erreur left">'.$erreur['datec'][0].'</p>';?></li>
						<li  class="exped" ><input type="text" value="<?php if(isset($_POST['exp'][0]) && $_POST['exp'][0]!='') {echo $_POST['exp'][0];} ?>" name="exp[]" id="exp1" class="expcourrier" /><?php if(isset($erreur['exp'][0])) echo '<p class="erreur left">'.$erreur['exp'][0].'</p>';?></li>
						<li  class="exped" ><input type="text" value="<?php if(isset($_POST['contenu'][0]) && $_POST['contenu'][0]!='') {echo $_POST['contenu'][0];} ?>" name="contenu[]" id="contenu1" class="contenucourrier"/><?php if(isset($erreur['contenu'][0])) echo '<p class="erreur left">'.$erreur['contenu'][0].'</p>';?></li>
						<li class="classdate" ><input type="text" value="<?php if(isset($_POST['tech'][0]) && $_POST['tech'][0]!='') {echo $_POST['tech'][0];}?>" name="tech[]" id="tech1" /><?php if(isset($erreur['tech'][0])) echo '<p class="erreur left">'.$erreur['tech'][0].'</p>';?></li>
						<li></li>
					</ul>
				</li>
				<?php
			...
			<input type="hidden" id="nblignes" name="nblignes" value="<?php if(isset($_POST['nblignes']) && $_POST['nblignes']!=0) echo $_POST['nblignes']; else echo '1'; ?>" />
